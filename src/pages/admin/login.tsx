...
import { Shield, Layers, Lock, LogOut, AlertTriangle, Loader2, KeyRound } from "lucide-react";
      ...
      >
        ...
        <div className="rounded-2xl border border-white/10 bg-white/4 backdrop-blur-xl p-8 shadow-2xl">
          ...
          <button
            onClick={() => { void signin(); }}
            disabled={isLoading}
            className="w-full h-11 rounded-xl bg-primary hover:bg-primary/90 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-primary/20 mb-3"
          >
            {isLoading ? (

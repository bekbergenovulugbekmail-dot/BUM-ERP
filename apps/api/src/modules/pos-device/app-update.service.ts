...
export type DesktopUpdate = {
  ...
  url: string | null;
  sha256: string | null;
  /** Ed25519 imzo — kassa o'z ichidagi ochiq kalit bilan tekshiradi. */
  signature: string | null;
  notes: string | null;
};

/** Platforma admini e'lon qilgan reliz (bazada) — muhit o'zgaruvchilaridan ustun. */
export type PublishedRelease = { id: string; version: string; sha256: string; signature: string | null; notes: string | null; minVersion: string | null };
...
export function desktopUpdate(current: string | undefined, env: NodeJS.ProcessEnv = process.env, release: PublishedRelease | null = null): DesktopUpdate {
  ...
  if (!SEMVER.test(latest) || !url.startsWith("https://") || !/^[a-f0-9]{64}$/.test(sha256)) {
    return { configured: false, available: false, mandatory: false, current: version, latest: null, url: null, sha256: null, signature: null, notes: null };
  }
  return {
    configured: true,
    ...
    url,
    sha256,
    // Tashqi manzildagi reliz imzosi (release-sign.mjs) — bo'lmasa kassa o'rnatmaydi
    signature: env.DESKTOP_SIGNATURE?.trim() || null,

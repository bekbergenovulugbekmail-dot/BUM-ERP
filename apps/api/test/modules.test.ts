...
const setModule = (cookie: string, key: string, enabled: boolean, extra: object = {}) =>
  call(cookie, "PUT", `/api/company/modules/${key}`, { enabled, ...extra });
...
describe("Modullar: server guard, bog'liqliklar, RBAC, obuna, ro'yxatdan o'tish", () => {
  it("o'chirish: API MODULE_DISABLED, ma'lumot saqlanadi, boshqa kompaniya ta'sirlanmaydi, qayta yoqish; tarix va audit", async () => {
    ...
    const off = await setModule(a.ownerCookie, "finance", false, { reason: "Hozircha kerak emas" });
    ...
    expect((await setModule(a.ownerCookie, "finance", false)).json().changed).toBe(false);
    expect((await setModule(a.ownerCookie, "finance", true)).statusCode).toBe(200);
    ...
    expect(history.map((row) => `${row.moduleKey}:${row.enabled}:${row.source}`).sort()).toEqual(["finance:false:owner", "finance:true:owner"]);
    ...
    expect(view.history.find((row: { enabled: boolean }) => !row.enabled)).toMatchObject({ reason: "Hozircha kerak emas", source: "owner" });
  });
  ...
  it("bog'liqliklar va tizim qismlari; POS o'chsa POS API, qurilma sinxroni va yangi qurilma yopiladi", async () => {
    ...
    const products = await setModule(a.ownerCookie, "products", false);
    ...
    for (const core of ["dashboard", "subscription", "settings", "auth", "company"]) {
      expect((await setModule(a.ownerCookie, core, false)).statusCode, core).toBe(400);
    }

    expect((await setModule(a.ownerCookie, "sales", false)).json().details.dependents).toEqual(expect.arrayContaining(["distribution", "delivery"]));
    for (const key of ["delivery", "distribution", "sales"]) expect((await setModule(a.ownerCookie, key, false)).statusCode, key).toBe(200);
    const needsSales = await setModule(a.ownerCookie, "delivery", true);
    ...
    expect((await setModule(a.ownerCookie, "pos", false)).statusCode).toBe(200);
  ...
  it("RBAC va obuna: xodim o'zgartira olmaydi; ruxsatsiz — FORBIDDEN, modul o'chiq — MODULE_DISABLED; obuna tugagani ustun; platforma admini", async () => {
    ...
    const denied = await setModule(kassir.cookie, "finance", false);
    ...
    expect((await setModule(a.ownerCookie, "finance", false)).statusCode).toBe(200);
    ...
    expect((await setModule(a.ownerCookie, "finance", true)).json().details).toMatchObject({ reason: "subscription_expired" });

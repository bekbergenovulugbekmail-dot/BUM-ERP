...
const SYNC_TONE: Record<string, string> = { pending: "text-amber-600", rejected: "text-destructive", applied: "text-emerald-600", discarded: "text-muted-foreground" };
    ...
    <dl className="divide-y divide-border text-sm">
      ...
      {report.shift.closingCash !== null && (
        <div className="flex justify-between gap-3 py-1.5">
          ...
          <dd className={`tabular-nums ${num(report.difference) === 0 ? "text-emerald-600" : "text-amber-600"}`}>
          ...
          {report ? (
            <div className="rounded-2xl border border-border bg-card p-4">
              <div className="mb-2 flex items-center justify-between">
                ...
                {report.unsynced > 0 && <span className="text-xs text-amber-600">{report.unsynced} ta hujjat serverga yuborilmagan</span>}
              </div>
              ...
              {entries.map((entry) =>
                entry.kind === "movement" ? (
                  <li key={entry.movement.id} className="flex items-center gap-3 px-4 py-2 text-sm">
                    <span className={`w-16 text-xs font-semibold ${entry.movement.type === "in" ? "text-emerald-600" : "text-amber-600"}`}>
                  ...
                  <li key={entry.payment.id} className="flex items-center gap-3 px-4 py-2 text-sm">
                    <span className="w-16 text-xs font-semibold text-sky-600">MIJOZ</span>
                    <div className="min-w-0 flex-1">
                      <p className="font-medium">

...
export type SaleCalc = {
  ...
  payments: { method: PaymentMethod; tendered: bigint; paid: bigint; terminalId?: string; cashAccountId?: string }[];
  cashPaid: bigint;
  change: bigint;
  changeKept: bigint;
  foreign: ForeignPart[];
...
function paymentParts(
  ...
  due: bigint,
  errors: string[],
): { method: PaymentMethod; tendered: bigint; terminalId?: string; cashAccountId?: string }[] {
  ...
  // Server bilan bir xil: qism kaliti — usul + terminal + bank hisobi (UZCARD va HUMO — ikki qism, bitta terminal ikki marta — xato)
  const seen = new Set<string>();
  let known = 0n;
  let openParts = 0;
  for (const part of input.payments) {
    const key = `${part.method}|${part.terminalId ?? ""}|${part.cashAccountId ?? ""}`;
    if (seen.has(key)) {
      errors.push(
        part.terminalId
          ? "Bitta terminal to'lovi bir marta kiritiladi"
          : part.cashAccountId
            ? "Bitta bank hisobiga to'lov bir marta kiritiladi"
            : `${METHOD_NAMES[part.method] ?? part.method} to'lovi bir marta kiritiladi`,
      );
    }
    seen.add(key);
    if (part.terminalId && part.method !== "card") errors.push("Terminal faqat karta to'lovida tanlanadi");
    if (part.cashAccountId && part.method !== "bank") errors.push("Bank hisobi faqat bank to'lovida tanlanadi");
    if (part.amount === null) {
      openParts += 1;
      continue;
    }
  ...
  return input.payments.map((part) => {
    const terminal = { ...(part.terminalId ? { terminalId: part.terminalId } : {}), ...(part.cashAccountId ? { cashAccountId: part.cashAccountId } : {}) };

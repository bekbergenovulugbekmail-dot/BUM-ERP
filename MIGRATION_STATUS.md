...
- yangi kassa o'rnatish web'dan: Sozlamalar → "Kassa qurilmalari" → "Ilovani o'rnatish" — e'lon qilingan o'rnatuvchi (versiya, hajm, izoh, SHA-256) va "Yuklab olish" (`pos.devices.manage`, sessiya bilan). API: `GET /api/pos/devices` javobida `installer`, `GET /api/pos/devices/installer/:releaseId/download` (kassir — 403, kirishsiz — 401); nginx uzoq yuklab olish qoidasiga qo'shildi. Test `pos-deletions-releases` kengaytirildi
